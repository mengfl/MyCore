...
public class LoggerUtil {
     ...
     * @param value
     */
    public static void printAndWrite(String value){
        writeExceptionLog(value);
    }
    /**
     * 记录异常到文件
    ...
    public static void writeExceptionLog(String exceptionDescription) {
        //再判断文件是否存在
        File log=new File(DirectoryConstant.PATH_LOG.concat(File.separator).concat(DateUtil.getCurrentTime(false,new Date())).concat(".log"));
        if (!log.exists()) {
            try {
                log.createNewFile();
            } catch (IOException e) {
        ...
        if(log.exists())
        {
            try
            {
                ...
                fw.write("Time---" + DateUtil.getCurrentTime(new Date()) + "\r\n" + exceptionDescription + "\r\n\r\n");
                fw.close();
            }
            catch (Exception e) {
                e.printStackTrace();
    ...
    public static void writeCrashLog(String exceptionDescription) {

        //再判断文件是否存在
        File log=new File(DirectoryConstant.PATH_CRASH.concat(File.separator).concat("crash:").concat(DateUtil.getCurrentTime(false,new Date())).concat(".log"));
        if (!log.exists()) {
            try {
                log.createNewFile();
            } catch (IOException e) {
                e.printStackTrace();
                LoggerUtil.writeExceptionLog(e.getMessage());
            }
        }
        //写入文件
        if(log.exists())
